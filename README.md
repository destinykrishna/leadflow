...
- **Interactive Lead Workspace**: Dedicated slide-over drawer and full-page workspace (`/app/leads/:id`) displaying borrower contact information, loan specifications, score meter, assigned advisor, and current stage. Explicitly surfaces "Already Known" borrower status (client/lead matches) with a direct link to the verified client case profile. Enforces linear stage transitions with HTTP 409 concurrency protection, integrates related tasks (`/api/tasks`) and client documents (`/api/documents`), and includes a client conversion modal with auto-generated credential display and 1-click clipboard copy.
